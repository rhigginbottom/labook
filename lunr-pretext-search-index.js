...
},
{
  ...
  "body": " To Students and Instructors  This book was first written for Washington & Jefferson College (W&J) students. At W&J, we teach Linear Algebra not only for its content but also (a) as a second course-length exposure to mathematical writing, and (b) as an introduction to the abstract and axiomatic thinking that is characteristic of higher mathematics. This initial audience and purpose has shaped this book.  As a text to be used in a second writing course, we have not included explicit writing instruction in the text. While training in logic and proof techniques is assumed from the beginning, we take care to identify proof strategies being used in these pages—especially in the early chapters. Along these lines, in most sections we have identified exercises as writing exercises which students should prioritize (and which instructors should consider requiring).  The fact that this text is written to introduce students to abstract concepts has influenced the choice and order of content. We introduce fields before vector spaces, and we introduce both structures earlier than many other texts. Theorems are then stated for general fields (instead of just for or ) throughout the text until the topic requires a restriction—such as inner products in . To emphasize that not all fields should be thought of as , finite fields, the complex numbers, and other fields show up explicitly as examples. (Readers should note that care must be taken when using calculators\/software\/online aids to row-reduce matrices over fields which are not . Some pointers are provided in .)   Other Features of this Book  Each section of this book contains Reading Questions . Ideally, instructors will assign and students will complete these questions before the class period in which material from that section of the book will be covered. The main goals of these questions is to force students to read the text and to guide a somewhat thoughtful first exposure to the content. We have found it helpful to emphasize to students that we do not expect them to fully understand the material upon this first reading, but that multiple exposures are often needed for comprehension and it is helpful if the classroom exposure is the second encounter, not the first. (If the instructor can skim the answers to these reading questions before class, then in-class examples and explanations can be customized accordingly.)  We have designated some problems at the end of each section as Writing Exercises . These are collected to help students practice and develop their writing. Some of these problems might be good to discuss in groups during class, and some might make useful homework exercises. As mentioned above, the separation of these problems is driven by one of the original purposes of this text—to help students improve their mathematical writing.  Many of the problems at the end of the sections have answers or solutions included in . Readers will notice that not all problems have answers or solutions written out—some of this incompleteness is due to the fact that this book is still a work in progress, some is because we required students to turn in written solutions to specific problems and did not want those solutions staring our students in the face.  Finally, a word on the title. Virtually every Linear Algebra text is titled, Linear Algebra . That's fine; textbook titles are not, perhaps, where an author's greatest skill and creativity are deployed. The subtitle of this book was chosen with the metaphor of an apprentice handyman in mind. Such a worker needs to acquire some tools and improve their skill in using others. Additional implements may need sharpening or other maintenance. Linear Algebra is used widely in mathematics, and for a book pitched at intermediate-level undergraduate students of mathematics, this metaphor seemed appropriate. Plus, a good acronym which sounds somewhat French (LAGAST!) is hard to resist.   "
},
{
...
},
{
  ...
  "number": "1.1.9",
  "title": "",
  "body": "  We first consider the linear system consisting of two simple equations where the solution to each equation is a line in : . Readers will likely realize that the solution to this system happens when the graphs of these lines intersect. We see from the graph below that the intersection occurs at the point . Since this is the only intersection point, there is only one solution to this system.   A graph of two intersecting lines   The graph of two lines on coordinate axes. The lines have slopes 1 and -2 and meet at the point (2,1). See long description.   The graph of two lines plotted on coordinate axes. The horizontal axis ranges from to and the vertical axis ranges from to . One line has slope and -intercept and the other line has slope and -intercept . The lines intersect at the point . The lines are labeled with their defining equations.      We now consider a second linear system: . The graphs of these two lines appear below, but in this case the lines do not intersect at all because they are parallel. (The slope of both lines is .)   A graph of two parallel lines   The graph of two lines on coordinate axes. The lines both have slope -2 and are parallel. See long description.   The graph of two lines plotted on coordinate axes. The horizontal axis ranges from to and the vertical axis ranges from to . One line has slope and -intercept and the other line has slope and -intercept . The lines are parallel and do not intersect. The lines are labeled with their defining equations.      Here is a third linear system in two variables: . Each of these equations has a solution set whose graph is a line. In this particular case, we obtain the same line for both equations, which means that the linear system has an infinite number of solutions. Each one of the infinite solutions to the first equation is a solution to the second equation, and vice versa.   A graph of two identical lines   The graph of one line on coordinate axes. The line has slope -2 and y intercept 5. See long description.   The graph of one lines plotted on coordinate axes. The horizontal axis ranges from to and the vertical axis ranges from to . The line has slope and -intercept . The line is bold indicating that both equations produce the same graph. The line is labeled with its defining equation.       "
},
{
  "id": "def-consistent",
  "level": "2",
...
},
{
  ...
  "number": "1.1.2",
  "title": "",
  "body": "  Suppose that is a function whose graph passes through the points , , and . What are the values of , , and ?  Model this problem with a linear system. (You do not need to find the solution set of this linear system.)    The linear system to solve is .   "
},
{
...
},
{
  ...
  "number": "1.1.5",
  "title": "",
  "body": "  Determine whether or not each given list is a solution to this linear system. .                           No.    No.    No.      "
},
{
  ...
  "number": "1.1.6",
  "title": "",
  "body": "  For the following linear systems, graph the solution set of each equation. Then graph the solution set of the linear system.                                       The solution set is .    The solution set is empty.    The solution set is empty.    The solution set is .    The solution set is .      "
},
{
...
},
{
  ...
  "number": "1.1.8",
  "title": "",
  "body": "  Consider a linear system of equations and variables and suppose that and are both solutions to this system. Under what conditions is also a solution to the same system? Explain.    This is true if and only if the system is a homogeneous linear system.   "
},
{
  ...
  "number": "1.1.9",
  "title": "",
  "body": "  Consider a linear system of equations and variables and suppose that and are both solutions to this system. Let be a real number. Prove that is also a solution to this same linear system.   "
},
{
  "id": "sec-matrices",
  "level": "1",
...
},
{
  ...
  "number": "1.2.10",
  "title": "",
  "body": " The reader should observe that every matrix which is in RREF is automatically in REF, but the converse is not true.  "
},
{
  "id": "sec-matrices-24",
  "level": "2",
...
},
{
  ...
  "number": "1.2.3",
  "title": "",
  "body": "  Put the following matrix into reduced row-echelon form. Record each step along the way (both the elementary row operations used and the matrices obtained as a result): .   "
},
{
  "id": "sec-matrices-31-1",
  "level": "2",
  "url": "sec-matrices.html#sec-matrices-31-1",
  "type": "Exercise",
  "number": "1.2.1",
  "title": "",
...
{
  "id": "sec-matrices-31-2",
  "level": "2",
  "url": "sec-matrices.html#sec-matrices-31-2",
  "type": "Exercise",
  "number": "1.2.2",
  "title": "",
...
{
  "id": "sec-matrices-31-3",
  "level": "2",
  "url": "sec-matrices.html#sec-matrices-31-3",
  "type": "Exercise",
  "number": "1.2.3",
  "title": "",
...
{
  "id": "sec-matrices-31-4",
  "level": "2",
  "url": "sec-matrices.html#sec-matrices-31-4",
  "type": "Exercise",
  "number": "1.2.4",
  "title": "",
  "body": "  For each of the following matrices, determine if the matrix is in row-echelon form, reduced row-echelon form, both, or neither.                             Neither    REF    REF    Both      "
},
{
  "id": "sec-matrices-31-5",
  "level": "2",
  "url": "sec-matrices.html#sec-matrices-31-5",
  "type": "Exercise",
  "number": "1.2.5",
  "title": "",
  "body": "  Consider the following matrix : . Find two distinct matrices and which are row equivalent to and are in row echelon form. (There are many correct answers!)    Two possibilities (there are many!): .   "
},
{
  "id": "sec-matrices-31-6",
  "level": "2",
  "url": "sec-matrices.html#sec-matrices-31-6",
  "type": "Exercise",
  "number": "1.2.6",
  "title": "",
...
{
  "id": "sec-matrices-31-7",
  "level": "2",
  "url": "sec-matrices.html#sec-matrices-31-7",
  "type": "Exercise",
  "number": "1.2.7",
  "title": "",
  "body": "  For each of the following matrix sizes, list all of the possible RREFs for matrices of that size. Use the symbols for a pivot, for an unspecified number, and for a zero entry.                        There are 4 possible RREFs.    There are 7 possible RREFs.    There are 4 possible RREFs.      "
},
{
  "id": "sec-matrices-31-8-2",
  "level": "2",
  "url": "sec-matrices.html#sec-matrices-31-8-2",
  "type": "Exercise",
  "number": "1.2.8",
  "title": "",
...
{
  "id": "sec-matrices-31-8-5",
  "level": "2",
  "url": "sec-matrices.html#sec-matrices-31-8-5",
  "type": "Exercise",
  "number": "1.2.11",
  "title": "",
...
},
{
  ...
  "number": "1.3.10",
  "title": "",
  "body": "  In a consistent linear system, all solutions can be expressed by writing the basic variables as linear functions of the free variables (if they exist). Further, each basic variable can be written as a linear function of only those free variables with .    The first sentence in this lemma has essentially been proved in the discussion within . We will prove the second statement directly. We consider as the augmented matrix of a consistent linear system. Suppose is row equivalent to , where is in RREF. We recall that part of the definition of RREF ( ) is that pivots are the leftmost non-zero number in their row.  We consider the linear equation corresponding to row of ; this equation will begin with a basic variable and will possibly involve other variables , with , before the equals sign. However, all of these other variables will be free variables, because any other basic variable , with , will correspond to a column in which that pivot is the only non-zero number. In other words, all entries along row in which correspond to pivot columns , for , will be zero.   "
},
{
...
},
{
  "id": "sec-results-29-1",
  "level": "2",
  "url": "sec-results.html#sec-results-29-1",
  "type": "Reading Question",
  "number": "1.3.1",
  "title": "",
...
},
{
  "id": "sec-results-29-2",
  "level": "2",
  "url": "sec-results.html#sec-results-29-2",
  "type": "Reading Question",
  "number": "1.3.2",
  "title": "",
...
},
{
  "id": "sec-results-30-1",
  "level": "2",
  "url": "sec-results.html#sec-results-30-1",
  "type": "Exercise",
  "number": "1.3.1",
  "title": "",
  "body": "  In each of the following, suppose the augmented matrix for a linear system has been reduced to the following RREF. Write down the solution(s) to the system (if they exist).                             The only solution is .    The solutions can be written parametrically as     There are no solutions.    The solutions can be written parametrically as       "
},
{
  "id": "sec-results-30-2",
  "level": "2",
  "url": "sec-results.html#sec-results-30-2",
  "type": "Exercise",
  "number": "1.3.2",
  "title": "",
...
},
{
  "id": "sec-results-30-3",
  "level": "2",
  "url": "sec-results.html#sec-results-30-3",
  "type": "Exercise",
  "number": "1.3.3",
  "title": "",
...
},
{
  "id": "sec-results-30-4",
  "level": "2",
  "url": "sec-results.html#sec-results-30-4",
  "type": "Exercise",
  "number": "1.3.4",
  "title": "",
...
},
{
  "id": "sec-results-30-5",
  "level": "2",
  "url": "sec-results.html#sec-results-30-5",
  "type": "Exercise",
  "number": "1.3.5",
  "title": "",
  "body": "  Under what conditions is the following linear system consistent? Your answer should be an equation or equations that must be satisfied by the . Explain your answer.     This system is consistent if and only if .   "
},
{
  "id": "sec-results-30-6",
  "level": "2",
  "url": "sec-results.html#sec-results-30-6",
  "type": "Exercise",
  "number": "1.3.6",
  "title": "",
...
},
{
  "id": "sec-results-30-7",
  "level": "2",
  "url": "sec-results.html#sec-results-30-7",
  "type": "Exercise",
  "number": "1.3.7",
  "title": "",
...
},
{
  "id": "sec-results-30-8",
  "level": "2",
  "url": "sec-results.html#sec-results-30-8",
  "type": "Exercise",
  "number": "1.3.8",
  "title": "",
  "body": "  Answer the question posed in by solving the linear system that was created in that exercise.    The function is .   "
},
{
  "id": "sec-results-30-9",
  "level": "2",
  "url": "sec-results.html#sec-results-30-9",
  "type": "Exercise",
  "number": "1.3.9",
  "title": "",
  "body": "  Suppose that the graph of the function passes through the points , , , and . Determine the values of , , , and .    The values are , , , and .   "
},
{
  ...
  "number": "1.3.10",
  "title": "",
  "body": "  Prove that every homogeneous linear system is consistent.    It is easy to find at least one solution to every homogeneous system. Why?   "
},
{
  "id": "exer-2b2-det",
  "level": "2",
...
},
{
  ...
  "number": "1.3.12",
  "title": "",
  "body": "  Using only results from this section, explain why every linear system over has either no solutions, exactly one solution, or an infinite number of solutions.   "
},
{
  "id": "sec-results-30-10-5",
  "level": "2",
  "url": "sec-results.html#sec-results-30-10-5",
  "type": "Exercise",
  "number": "1.3.13",
  "title": "",
  "body": "  Suppose that a coefficient matrix for a linear system has three pivot columns. Is the system consistent? Explain.    Yes, this system is consistent. (Why?)   "
},
{
  "id": "sec-results-30-10-6",
  "level": "2",
  "url": "sec-results.html#sec-results-30-10-6",
  "type": "Exercise",
  "number": "1.3.14",
  "title": "",
  "body": "  What would one have to know about the pivot columns in an augmented matrix in order to conclude that the linear system has exactly one solution? Explain.   "
},
{
  "id": "exer-same-pivots",
  "level": "2",
...
},
{
  ...
  "number": "1.4.11",
  "title": "",
  "body": " Illustration of two vectors in the plane   Graph of two vectors on coordinate axes. Both vectors begin at the origin ending at (1,-2) and (-3,1). See long description.   The graph of two vectors plotted on coordinate axes. The horizontal axis ranges from to and the vertical axis ranges from to . The vector is drawn in red and begins at the origin extending to . The vector is drawn in blue and begins at the origin extending to the point .     "
},
{
  ...
  "number": "1.4.12",
  "title": "",
  "body": " Illustration of scalar multiplication   Graph of three vectors on axes, starting at the origin ending at (-3,1), (-1.5,0.5), and (3,-1). See long description.   The graph of three vectors plotted on coordinate axes. The horizontal axis ranges from to and the vertical axis ranges from to . The vector is drawn in red and begins at the origin extending to . The vector is drawn in blue and begins at the origin extending to the point . The vector is drawn in green and begins at the origin extending to the point .     "
},
{
  ...
  "number": "1.4.13",
  "title": "",
  "body": " Illustration of vector addition   The graph of four vectors; three vectors go from (0,0) to (-3,1), (1,-2), and (-2,-1). See long description.   The graph of four vectors plotted on coordinate axes. The horizontal axis ranges from to and the vertical axis ranges from to . The vector is drawn in red and begins at the origin extending to . The vector is drawn in blue and begins at the origin extending to the point . A vector drawn in dashed red extends from to ; this vector is labeled . The vector is drawn in black begins at the origin and extends to .     "
},
{
  ...
  "number": "1.4.14",
  "title": "",
  "body": " Illustration of a linear combination   The graph of four vectors; three vectors go from (0,0) to (-1.6,3.2), (2,2), and (3.6,-1.2). See long description.   The graph of four vectors plotted on coordinate axes. The horizontal axis ranges from to and the vertical axis ranges from to . The vector is drawn in red and begins at the origin extending to . The vector is drawn in blue and begins at the origin extending to the point . A vector drawn in dashed red extends from to ; this vector is labeled . The vector is drawn in black begins at the origin and extends to .     "
},
{
  "id": "sec-vectors-5-1",
  "level": "2",
...
},
{
  ...
  "number": "1.4.4.1",
  "title": "",
  "body": "  Write a linear system that is equivalent to the following vector equation:     Here is the linear system: .   "
},
{
  "id": "sec-vectors-6-2",
  "level": "2",
  ...
  "number": "1.4.4.2",
  "title": "",
  "body": "  Write a vector equation that is equivalent to the following linear system: .    Here is the vector equation: .   "
},
{
  "id": "sec-vectors-6-3",
  "level": "2",
  ...
  "number": "1.4.4.3",
  "title": "",
  "body": "  Let and be the following vectors: . List five vectors in . For each vector, write down the specific linear combination of and used to generate the vector.    There are an infinite number of correct answers! Three vectors that are in the span are , , and . Here are two additional vectors in the span: .   "
},
{
  "id": "sec-vectors-6-4",
  "level": "2",
  ...
  "number": "1.4.4.4",
  "title": "",
  "body": "  Describe all possible ways of writing as a linear combination of , , and .    If , , , and , then can be written as , where , , and can be any real number.   "
},
{
  "id": "sec-vectors-6-5",
  "level": "2",
...
},
{
  ...
  "number": "1.4.4.7",
  "title": "",
  "body": "  Let , , and be the following vectors: . Under what conditions is in ? This will be an equation (or equations) satisfied by the . Explain your answer.    The vector is in as long as the following equation is satisfied: .   "
},
{
...
},
{
  ...
  "number": "1.4.4.9",
  "title": "",
  "body": "  Two vectors and in are called collinear if or for some . Show that the span of any two nonzero vectors in which are not collinear is all of .    One might try to prove that and are collinear if and only if . Then an exercise from section 1.3 can be invoked.   "
},
{
...
},
{
  ...
  "number": "1.4.4.12",
  "title": "",
  "body": "  Prove property 5 from . Note which properties of the real numbers you use in this proof.    We must prove that for all real numbers and all and in we have . Let and be denoted in the following way: . Then we have . Now, since the distributive law holds for real numbers, we have for all . This means that . This completes the proof.   "
},
{
  "id": "sec-fields",
  "level": "1",
...
},
{
  "id": "sec-fields-8",
  "level": "2",
  "url": "sec-fields.html#sec-fields-8",
  "type": "Note",
  "number": "2.1.2",
  "title": "",
  "body": " Axioms 5 and 6 could perhaps be stated with a bit more precision. Some texts state axiom 5 like this: the set must have an additive identity, that is, an element such that for all . Those same texts may also state axiom 6 without reference to the symbol : for each , there exists an element such that , where is the additive identity of the field.  For some students, the use of the symbol might be helpful, reminding them of the essential property of the additive identity through familiarity. But this may be confusing when handling a set which contains the symbol when that symbol does not function as the relevant additive identity.  Our point here is that the use of in the axioms for a field (and some of the subsequent theorems) is intended to stand in for a generic symbol for the additive identity. Similarly, the symbol stands in as the generic symbol for the multiplicative identity. (A reader could imagine a restatement of axioms 9 and 10 above using a letter as the multiplicative identity in the way we have demonstrated axioms 5 and 6 can be restated.)  The reader should also note that in a generic field the term nonzero means not the additive identity.   "
},
{
  "id": "pars-axioms-2",
  "level": "2",
...
},
{
  "id": "sec-fields-11",
  "level": "2",
  "url": "sec-fields.html#sec-fields-11",
  "type": "Example",
  "number": "2.1.3",
  "title": "",
...
},
{
  ...
  "number": "2.1.8",
  "title": "",
  "body": "  In this problem we consider adding an element to .   If is defined by is a field? Justify your answer.    If is defined by is a field? Justify your answer.              Yes, this is a field. The most difficult axiom to check is the one about multiplicative inverses. It turns out that the multiplicative inverse of is where . One needs to check that for all non-zero elements of we have . But this is relatively easy to verify.      "
},
{
  "id": "exer-q-root-2",
  "level": "2",
...
},
{
  ...
  "number": "2.2.11",
  "title": "",
  "body": "  Give descriptions of linear systems with each of the following properties, or state why such a system is impossible. Specific numbers and equations are not necessary, but your description should include what field is involved, the number of equations and variables, the number of free variables, etc. Explain your answers.   A consistent system with exactly 8 solutions    A consistent system with exactly 10 solutions    A consistent system with exactly 9 solutions    A consistent system with exactly 17 solutions      "
},
{
...
},
{
  "id": "matrices-vspace",
  "level": "2",
  "url": "sec-vector-spaces.html#matrices-vspace",
  "type": "Example",
  "number": "2.3.10",
  "title": "",
  "body": "  If we fix positive integers and and a field , then is the set of all matrices with entries in . When , we use the notation .  With the standard addition and scalar multiplication of matrices (see for a description), the set is a vector space over for a fixed and . The quick explanation here is that because of the way scalar multiplication and addition of matrices are defined, once a reader believes that is a vector space (see ), believing that is a vector space just amounts to considering several columns at the same time. But since these columns have no interaction with each other as far as these operations are concerned, none of the vector space axioms will be violated. The zero vector for this vector space is the zero matrix.   "
},
{
...
{
  "id": "sec-vector-spaces-18-1",
  "level": "2",
  "url": "sec-vector-spaces.html#sec-vector-spaces-18-1",
  "type": "Reading Question",
  "number": "2.3.1",
  "title": "",
...
{
  "id": "sec-vector-spaces-18-2",
  "level": "2",
  "url": "sec-vector-spaces.html#sec-vector-spaces-18-2",
  "type": "Reading Question",
  "number": "2.3.2",
  "title": "",
...
{
  "id": "sec-vector-spaces-18-3",
  "level": "2",
  "url": "sec-vector-spaces.html#sec-vector-spaces-18-3",
  "type": "Reading Question",
  "number": "2.3.3",
  "title": "",
...
},
{
  ...
  "number": "2.3.1",
  "title": "",
  "body": "  For each of the following, determine whether the given subset is a real vector space, using the usual operations of vector addition and scalar multiplication in . (By a real vector space we simply mean a vector space over the field .)                        No, this is not a vector space. For example, it is not closed under scalar multiplication. We can see that .    No, this is not a vector space. For example, it is not closed under addition. We can see that .    Yes, this is a vector space. All of the vector space axioms hold.      "
},
{
  "id": "exercises-2-3-2",
  "level": "2",
...
},
{
  ...
  "number": "2.3.3",
  "title": "",
  "body": "  Consider the set with the usual vector addition but with scalar multiplication defined this way: . This is not a vector space over . Determine which of the vector space axioms fail, and give an explanation for each such axiom.    The only axiom that fails is axiom 7. We can see, for example, that , so this axiom does not hold.   "
},
{
  "id": "exercises-2-3-4",
  "level": "2",
  ...
  "number": "2.3.4",
  "title": "",
  "body": "  Consider the following three elements of : . Calculate the linear combination of , , and with weights , , and , respectively.    We calculate that .   "
},
{
  "id": "exercises-2-3-5",
  "level": "2",
...
},
{
  ...
  "number": "2.3.6",
  "title": "",
  "body": "  Consider the vector space .   How many distinct vectors are in ?    If and are the following vectors, , how many distinct vectors are in ?          This vector space contains distinct vectors.    The set contains four distinct vectors: , , , and . These are all distinct.      "
},
{
  "id": "exercises-2-3-7",
  "level": "2",
...
},
{
  ...
  "number": "2.3.9",
  "title": "",
  "body": "  Consider the following vectors in : . Is ? Explain your answer.    No, is not in . When we form the matrix with , , and as its columns, it reduces to the following: . Since there is a pivot in the final column, the corresponding vector equation has no solutions.   "
},
{
  "id": "exercises-2-3-10-2",
  "level": "2",
  "url": "sec-vector-spaces.html#exercises-2-3-10-2",
  "type": "Exercise",
  "number": "2.3.10",
  "title": "",
...
{
  "id": "exercises-2-3-10-3",
  "level": "2",
  "url": "sec-vector-spaces.html#exercises-2-3-10-3",
  "type": "Exercise",
  "number": "2.3.11",
  "title": "",
...
{
  "id": "exercises-2-3-10-4",
  "level": "2",
  "url": "sec-vector-spaces.html#exercises-2-3-10-4",
  "type": "Exercise",
  "number": "2.3.12",
  "title": "",
  "body": "  Is a vector space over ? (Addition and scalar multiplication should be understood as the obvious operations in .) Explain your answer.    No, is not a vector space over . It is not closed under scalar multiplication. For example, we see that .   "
},
{
  "id": "exercises-2-3-10-5",
  "level": "2",
  "url": "sec-vector-spaces.html#exercises-2-3-10-5",
  "type": "Exercise",
  "number": "2.3.13",
  "title": "",
...
{
  "id": "exercises-2-3-10-6",
  "level": "2",
  "url": "sec-vector-spaces.html#exercises-2-3-10-6",
  "type": "Exercise",
  "number": "2.3.14",
  "title": "",
...
{
  "id": "exercises-2-3-10-7",
  "level": "2",
  "url": "sec-vector-spaces.html#exercises-2-3-10-7",
  "type": "Exercise",
  "number": "2.3.15",
  "title": "",
  "body": "  Prove in .    Suppose that is a zero vector in and that also has the properties of a zero vector. Since is a zero vector, we have . Since has the properties of a zero vector, we have . Therefore, we have , so . This proves that the zero vector is unique.   "
},
{
  "id": "exercises-2-3-10-8",
  "level": "2",
  "url": "sec-vector-spaces.html#exercises-2-3-10-8",
  "type": "Exercise",
  "number": "2.3.16",
  "title": "",
  "body": "  Prove in .   "
...
{
  "id": "exercises-2-3-10-9",
  "level": "2",
  "url": "sec-vector-spaces.html#exercises-2-3-10-9",
  "type": "Exercise",
  "number": "2.3.17",
  "title": "",
  "body": "  Prove in .   "
...
},
{
  ...
  "number": "2.4.2",
  "title": "",
  "body": "  For each of the following, determine whether or not the subset of is a subspace of . Explain your answers.                                  Yes, is a subspace.    Yes, is a subspace.    No, is not a subspace. It is not closed under scalar multiplication.    Yes, is a subspace.    Yes, is a subspace.      "
},
{
  ...
  "number": "2.4.3",
  "title": "",
  "body": "  Let be the vector space of continuous functions defined , and let be the set of differentiable functions defined . (The fact that is a vector space is established in .)  For each of the following, determine whether or not the subset of is a subspace of . Explain your answers.                              "
},
{
  ...
  "number": "2.4.4",
  "title": "",
  "body": "  For each of the following, determine whether or not the subset of is a subspace of . Explain your answers. (We are writing vectors horizontally for convenience.)                             Yes, is a subspace as it can be written as the span of two vectors.    No, is not a subspace. It is not closed under scalar multiplication, for example.    Yes, is a subspace as it can be written as the span of four vectors.    No, is not a subspace as it does not contain the zero vector.      "
},
{
...
},
{
  ...
  "number": "2.4.6",
  "title": "",
  "body": "  Let and be real numbers with . Show that the set of all functions such that is a subspace of . (See for the definition of .)    Let denote the set of functions described in the statement of the problem. We first observe that the zero function is in since it integrates to zero over any interval . If , then , which proves that . Finally, if and , we have , which shows that . (These last two calculations rely on the linear properties of the definite integral.) This completes the proof that is a subspace.   "
},
{
...
},
{
  ...
  "number": "2.4.9",
  "title": "",
  "body": "  Let be a vector space and let and be subspaces of . Must be a subspace of ? Justify your answer.    No. Let and be the following two subspaces of : . Then and , so both of these elements are in . However, , and . This shows that is not closed under addition, so it is not a subspace.   "
},
{
...
{
  "id": "subsec-lin-trans-matrices-8",
  "level": "2",
  "url": "sec-linear-trans.html#subsec-lin-trans-matrices-8",
  "type": "Example",
  "number": "3.1.8",
  "title": "",
  "body": "  Let and be the following matrices over : . We note that both and are diagonal matrices and that is an identity matrix.  Building on , both and are linear transformations from to . Let be an element of where . Then we have . We also have . Since for all , the linear transformation acts as the identity function on . The action of on is slightly more complex, but only by a bit. Because of all the zeros in , the effect of the transformation is just multiplication of each coordinate of a vector in by the corresponding diagonal entry in . What we observe for multiplication by a diagonal or identity matrix in can easily be extended to for any and any field .   "
},
{
  "id": "note-define-ej",
  "level": "2",
...
{
  "id": "def-comp-linear-trans",
  "level": "2",
  "url": "sec-linear-trans.html#def-comp-linear-trans",
  "type": "Definition",
  "number": "3.1.10",
  "title": "",
  "body": "  If and are linear transformations between vector spaces, then the function defined by for each is the composition of the transformations and .   "
},
{
  ...
  "number": "3.1.11",
  "title": "",
  "body": " We note that is only possible when the codomain of is the same as the domain of . It is not difficult to show that is a linear transformation. (See .)  "
},
{
...
{
  "id": "subsec-props-linear-trans-6",
  "level": "2",
  "url": "sec-linear-trans.html#subsec-props-linear-trans-6",
  "type": "Example",
  "number": "3.1.13",
  "title": "",
...
{
  "id": "subsec-props-linear-trans-9",
  "level": "2",
  "url": "sec-linear-trans.html#subsec-props-linear-trans-9",
  "type": "Note",
  "number": "3.1.15",
  "title": "",
  "body": " In , we usually use the notation in place of . Considered only as a function, is unique and therefore using is not ambiguous.  "
},
{
  "id": "def-isomorphism",
  "level": "2",
...
},
{
  "id": "examp-r2-p1-iso",
  "level": "2",
  "url": "sec-linear-trans.html#examp-r2-p1-iso",
  "type": "Example",
  "number": "3.1.17",
  "title": "",
  "body": "  The vector spaces and are isomorphic. To prove this, we consider the function defined by . To justify the claim that and are isomorphic, we must prove that is a bijective linear transformation. We will prove that is a linear transformation and ask the reader to prove bijectivity in the exercises.  We let and be elements of where . Then we have , and . The properties possesses as a vector space show that .  We now suppose that and , where . Then . Again, we see that using the properties of as a vector space.  We have shown that is a linear transformation. It is also bijective (see ), so it is an isomorphism, meaning that and are isomorphic.   "
},
{
  "id": "notation-set-lts",
  "level": "2",
...
},
{
  ...
  "number": "3.1.19",
  "title": "",
  "body": "  Let . If is bijective, meaning exists, then is also a linear transformation.    We will check the two properties of a linear transformation. (See .) Suppose that . Since , we have . When we apply to the beginning and end of this equality, using , we get .  We will now check the scalar multiple property in a similar fashion. Let and let . Then we have . Applying to both sides again we get . This proves that .   "
},
{
  ...
  "number": "3.1.20",
  "title": "",
  "body": "  Let and be vector spaces over , and let . Then is an isomorphism if and only if is invertible.    Most of this result is true without worrying about any of the linear transformation properties. (A function is bijective if and only if it has an inverse.)  Here, then, is the brief argument in full. If is an isomorphism, then exists because is bijective. Then tells us that is a linear transformation. This shows that is invertible (see ). On the other hand, if is invertible, this implies both the existence of and the fact that is a linear transformation. This proves that is an isomorphism.   "
},
{
...
},
{
  ...
  "number": "3.1.6.2",
  "title": "",
  "body": "  Let , , , and be defined as follows: . Define a linear transformation to be multiplication by .   Find .     Find an in such that .     Is there more than one whose image under is ? How do you know?     Determine whether or not is in the image of . (Reminder: by image here we mean range .)       "
},
{
...
},
{
  ...
  "number": "3.1.7.2",
  "title": "",
  "body": "  Consider the function defined by . Is a linear transformation? Justify your answer.    Yes, is a linear transformation.   "
},
{
...
},
{
  ...
  "number": "3.1.7.4",
  "title": "",
  "body": "  Let be the function defined by . Is a linear transformation? Justify your answer.    No, is not a linear transformation. Consider the following example: . This shows that the scalar multiplication property does not hold, as .   "
},
{
...
},
{
  ...
  "number": "3.1.7.8",
  "title": "",
  "body": "  Let be the following matrix over : . Let be the linear transformation which is multiplication by .   Is the vector in the image of ? Explain your answer.    Is surjective? Explain your answer.          No, since . The pivot in the final column shows that there is no solution to the matrix-vector equation , where .    No. Since in part a we identified a vector which is not in the image of , this means that cannot be surjective.      "
},
{
...
},
{
  ...
  "number": "3.1.7.11",
  "title": "",
  "body": "  Define the function to be the following: . Prove that is a linear transformation. (See for a reminder of the definition of .)    Let and let . Then we have . We now let and . Then . Both of these calculations rely on the linear properties of the definite integral.   "
},
{
  ...
  "number": "3.1.7.12",
  "title": "",
  "body": "  Let and be linear transformations between vector spaces over a field . Prove that is also a linear transformation.    Let and be vectors in . Then, since and are both linear transformations, we have . This proves that has the first property of a linear transformation.  We now let and . Then, since and are both linear transformations, we have . This shows that is a linear transformation.   "
},
{
  ...
  "number": "3.1.7.13",
  "title": "",
  "body": "  Let and be linear transformations between vector spaces over a field .   Prove that if is injective, then must be injective.    Prove that if is surjective, then must be surjective.          We assume that is injective, and we let and be vectors in with . Then, since is injective, we have . This proves that is injective.    We assume that is surjective, and we let . Since is surjective, there is a vector such that . But this means that ; in other words, sends to . This proves that is surjective, since we have found a vector such that .      "
},
{
  "id": "lin-trans-zero",
  "level": "2",
...
{
  "id": "exer-prove-lt-bijective",
  "level": "2",
  "url": "sec-linear-trans.html#exer-prove-lt-bijective",
  "type": "Exercise",
  "number": "3.1.7.18",
  "title": "",
  "body": "  Finish the proof in that and are isomorphic. Specifically, prove that the linear transformation given in this example is bijective.   "
},
{
...
{
  "id": "exer-inv-iff-transpose-inv",
  "level": "2",
  "url": "sec-inverting-matrix.html#exer-inv-iff-transpose-inv",
  "type": "Exercise",
  "number": "3.3.5.9",
  "title": "",
  "body": "  Let . Prove that is invertible if and only if is invertible.   "
},
{
  "id": "exer-transpose-elem-is-elem",
  "level": "2",
  "url": "sec-inverting-matrix.html#exer-transpose-elem-is-elem",
  "type": "Exercise",
  "number": "3.3.5.10",
  "title": "",
  "body": "  Let be an elementary matrix. Prove that is also an elementary matrix.   "
},
{
...
{
  "id": "sec-inverting-matrix-7-8-7",
  "level": "2",
  "url": "sec-inverting-matrix.html#sec-inverting-matrix-7-8-7",
  "type": "Exercise",
  "number": "3.3.5.13",
  "title": "",
...
{
  "id": "sec-inverting-matrix-7-8-12",
  "level": "2",
  "url": "sec-inverting-matrix.html#sec-inverting-matrix-7-8-12",
  "type": "Exercise",
  "number": "3.3.5.18",
  "title": "",
  "body": "  How many matrices in are invertible? What proportion of the matrices in are invertible?   "
...
{
  "id": "sec-inverting-matrix-7-8-13",
  "level": "2",
  "url": "sec-inverting-matrix.html#sec-inverting-matrix-7-8-13",
  "type": "Exercise",
  "number": "3.3.5.19",
  "title": "",
...
},
{
  ...
  "number": "3.4.4.1",
  "title": "",
  "body": "  Consider the following matrix in : . For each of the following vectors , determine whether or not .                   Yes, we calculate .    No, we calculate , where we have written the vector horizontally out of convenience.      "
},
{
...
},
{
  ...
  "number": "3.4.4.4",
  "title": "",
  "body": "  Let and be the following, with entries from : .   Show that and .    Explain why it is not possible, for this particular matrix , to find a non-zero vector in .    Is it possible to find a matrix such that there exists a non-zero vector in ? Justify your answer thoroughly.          It is easy to calculate and see that . To show that is not in , we form the matrix and row reduce: . The pivot in the last column shows that .    Since , we can see that any vector in the null space of must be a multiple of . And since is not in , no non-zero multiple of can be in . This shows that the only vector in is the zero vector.    Yes, this is possible. Consider the matrix . The vector is in the null space of . (This can easily be checked.) Also, since is a column of , it is in .      "
},
{
...
},
{
  ...
  "number": "3.4.4.7",
  "title": "",
  "body": "  Consider the following linear transformation : .   Either prove or disprove that is injective.    Either prove or disprove that is surjective.          This linear transformation is injective. Observe that if sends the vector to the zero polynomial, then we have the following linear system in which each equation must hold: . We can solve this using methods from earlier in this book. We find that . This shows that the only solution to this linear system is the trivial one that is, where . But this means that the only vector that sends to the zero polynomial is the zero vector.  This proves that the kernel of contains only the zero vector, so is injective.    We claim that is surjective. Let be an arbitrary element of . In order to demonstrate that there is an element such that , we need to solve the following linear system: . However, the RREF of the coefficient matrix for this system looks like this: . The fact that there is a pivot in each row of this RREF means that a combination of , , and can be found such that . This proves that is surjective.      "
},
{
...
},
{
  ...
  "number": "3.4.4.9",
  "title": "",
  "body": "  Consider the following linear transformation : .   Either prove or disprove that is injective.    Either prove or disprove that is surjective.          This linear transformation is not injective. We can calculate that . Since this shows that has a nonzero vector in the kernel, is not injective.    This linear transformation is surjective. Let be an element of . We claim that we can find an element such that . If , we will need to solve the following system of equations to find the coefficients of : . But the coefficient matrix of this system has the following RREF: . The important thing to notice here is that there is a pivot in each row of this RREF. This means that the linear system always has a solution, so we can always find values of , , and such that . This proves that is surjective.      "
},
{
...
},
{
  ...
  "number": "3.4.4.11",
  "title": "",
  "body": "  Suppose that is a linear transformation between vector spaces over . If is a subspace of , prove that is a subspace of , where .    Since is a subspace of , we must have . Further, we know that for all linear transformations. This proves that .  We now let . This means that there exist with and . Since is a subspace of , we know that , so . However, because of the properties of a linear transformation, we have . This proves that , which shows that is closed under addition.  Finally, we let and . This means that there exists such that . Since is a subspace of , we know that , so . Using the properties of a linear transformation, we see that . This proves that , showing that is closed under scalar multiplication. This completes the proof that is a subspace of .   "
},
{
...
{
  "id": "subsec-det-alternative-8",
  "level": "2",
  "url": "sec-define-det.html#subsec-det-alternative-8",
  "type": "Note",
  "number": "4.1.10",
  "title": "",
...
{
  "id": "subsec-det-alternative-11",
  "level": "2",
  "url": "sec-define-det.html#subsec-det-alternative-11",
  "type": "Example",
  "number": "4.1.12",
  "title": "",
...
},
{
  ...
  "number": "4.1.3.2",
  "title": "",
  "body": "  Consider the following matrix : .   Compute the determinant of by cofactor expansion down the third column. Write out all of your calculations.     There is a better row or column to use for this calculation. Which one is it? Choose that row or column and compute the determinant of by cofactor expansion along that row or column. Write out all of your calculations. Why is this easier?       "
},
{
...
},
{
  ...
  "number": "4.1.4.6",
  "title": "",
  "body": "  Let be a matrix over and let . Find a formula that relates to and .    We note that , and .   "
},
{
  ...
  "number": "4.1.4.7",
  "title": "",
  "body": "  Let be a matrix over . Find a value of so that or explain why this is not possible.   "
},
{
  ...
  "number": "4.1.4.8",
  "title": "",
  "body": "  Let be the following matrix over : . Find a value of so that or explain why this is not possible.    We must have in order to have .   "
},
{
  ...
  "number": "4.1.4.9",
  "title": "",
  "body": "  Let be the following matrix over : . Find all values of such that .    The values are and .   "
},
{
...
},
{
  ...
  "number": "4.1.4.12",
  "title": "",
  "body": "  Suppose that is a matrix and that is the result of applying the replace row operation to . Prove that .    We let , and we consider the case of adding times row 1 to row 2. We will call the resulting matrix , and it has this appearance: . We know that , and we will now calculate : . Technically we have only proved one of the two necessary cases to complete this proof.   "
},
{
...
},
{
  ...
  "number": "4.2.4",
  "title": "",
  "body": "  Let and let be the result of multiplying a row in by a non-zero . Then .    We will not need induction for this argument. Suppose that is formed by multiplying row in by where . We will calculate by expanding along row . Note that since row is the only row affected by this operation, for all . Additionally, we note that for all . Now we have .   "
},
{
  "id": "thm-det-replace",
  "level": "2",
...
},
{
  ...
  "number": "4.2.13",
  "title": "",
  "body": "  Let . Then .    We will prove this in two cases. First, if is not invertible, then neither is , by . This means that since, by , both sides of the equation are zero.  If is invertible, then is row equivalent to , and there exist elementary matrices such that . In the calculation that follows, we use this factorization as well as repeated application of . We first use to peel the determinant of elementary matrices away from ; we then use the same result to put those determinants back together to form . Here is the argument: . This completes the proof.   "
},
{
...
{
  "id": "lem-el-mat-det-transpose",
  "level": "2",
  "url": "sec-det-properties.html#lem-el-mat-det-transpose",
  "type": "Lemma",
  "number": "4.2.15",
  "title": "",
  "body": "  If is an elementary matrix over a field , then .    We have three cases to consider, but two of the cases are immediate. If is an elementary matrix that performs the scale or switch row operation, then , so the result follows easily. (We ask the reader to prove this fact in .)  We now suppose that performs the replace row operation. We assume that performs the row operation of adding times row to row , where . This means that is the matrix with the extra feature of containing the entry in position . From , we know that , so we only need to prove that .  The matrix is except for the fact that it contains the element in position . We will calculate by using cofactor expansion along row : . This calculation reduces to one term because there is only one nonzero element in row of . (Choosing row for expansion means the element in position is removed when calculating the determinant of the submatrix.) Since , we have . Since and , this concludes our final case.   "
},
{
  "id": "subsec-proving-det-transpose-thm-6",
  "level": "2",
  "url": "sec-det-properties.html#subsec-proving-det-transpose-thm-6",
  "type": "Proof",
  "number": "4.2.3.1",
  "title": "Proof of Theorem 4.1.9.",
  "body": " Proof of  We assume that . If is not invertible, then is also not invertible (see ), meaning that both and by . This proves that .  We now assume that is invertible. Using and , we know that there exist elementary matrices such that . By repeated use of , we know that . We can take the transpose of both sides of , and using (part 4) repeatedly we have . We again use repeatedly (the transpose of an elementary matrix is an elementary matrix, see ), and we have . Finally, using and the fact that multiplication within is commutative, we conclude that .  "
},
{
  ...
  "number": "4.2.4.1",
  "title": "",
  "body": "  Consider the following three matrices: .   Calculate using cofactor expansion along some row or column. Show your work.     The matrix was obtained from by a single elementary row operation. Which one?     Knowing and given your answer to (b), what do you predict to be? (Consult .)     Calculate using cofactor expansion along some row or column. Show your work.     The matrix was obtained from by a single elementary row operation. Which one?     Knowing and given your answer to (e), what do you predict to be? (Consult .)     Calculate using cofactor expansion along some row or column. Show your work.       "
},
{
...
{
  "id": "sec-det-properties-7-1",
  "level": "2",
  "url": "sec-det-properties.html#sec-det-properties-7-1",
  "type": "Exercise",
  "number": "4.2.5.1",
  "title": "",
...
{
  "id": "sec-det-properties-7-2",
  "level": "2",
  "url": "sec-det-properties.html#sec-det-properties-7-2",
  "type": "Exercise",
  "number": "4.2.5.2",
  "title": "",
  "body": "  Find the determinant of the matrix using row reduction.                                 "
...
{
  "id": "sec-det-properties-7-3",
  "level": "2",
  "url": "sec-det-properties.html#sec-det-properties-7-3",
  "type": "Exercise",
  "number": "4.2.5.3",
  "title": "",
...
{
  "id": "sec-det-properties-7-4",
  "level": "2",
  "url": "sec-det-properties.html#sec-det-properties-7-4",
  "type": "Exercise",
  "number": "4.2.5.4",
  "title": "",
  "body": "  Find the determinant using a combination of row reduction and cofactor expansion: .         "
...
{
  "id": "sec-det-properties-7-5",
  "level": "2",
  "url": "sec-det-properties.html#sec-det-properties-7-5",
  "type": "Exercise",
  "number": "4.2.5.5",
  "title": "",
...
{
  "id": "sec-det-properties-7-6",
  "level": "2",
  "url": "sec-det-properties.html#sec-det-properties-7-6",
  "type": "Exercise",
  "number": "4.2.5.6",
  "title": "",
  "body": "  Calculate if .         "
...
{
  "id": "sec-det-properties-7-7",
  "level": "2",
  "url": "sec-det-properties.html#sec-det-properties-7-7",
  "type": "Exercise",
  "number": "4.2.5.7",
  "title": "",
...
{
  "id": "sec-det-properties-7-8-2",
  "level": "2",
  "url": "sec-det-properties.html#sec-det-properties-7-8-2",
  "type": "Exercise",
  "number": "4.2.5.8",
  "title": "",
  "body": "  Suppose that is a square matrix with two identical columns. Prove that .    If has identical columns, then has identical rows. We can use an elementary row operation to add times one of these rows to the other, producing a row of zeros in this matrix we will call . Since we used the replace row operation to go from to , we have by . Since has a row of zeros, we know that by . This means that , and since we have by , this means that , as desired.   "
},
{
  "id": "sec-det-properties-7-8-3",
  "level": "2",
  "url": "sec-det-properties.html#sec-det-properties-7-8-3",
  "type": "Exercise",
  "number": "4.2.5.9",
  "title": "",
...
{
  "id": "sec-det-properties-7-8-4",
  "level": "2",
  "url": "sec-det-properties.html#sec-det-properties-7-8-4",
  "type": "Exercise",
  "number": "4.2.5.10",
  "title": "",
...
{
  "id": "sec-det-properties-7-8-5",
  "level": "2",
  "url": "sec-det-properties.html#sec-det-properties-7-8-5",
  "type": "Exercise",
  "number": "4.2.5.11",
  "title": "",
  "body": "  Suppose that . Show that regardless of whether or not .    If , we have . This string of equations uses twice as well as the fact that the determinant of a matrix is an element of , and elements of commute via multiplication.   "
},
{
  "id": "sec-det-properties-7-8-6",
  "level": "2",
  "url": "sec-det-properties.html#sec-det-properties-7-8-6",
  "type": "Exercise",
  "number": "4.2.5.12",
  "title": "",
  "body": "  Let and let . Find a formula for and prove that your formula is correct.   "
},
{
  "id": "sec-det-properties-7-8-7",
  "level": "2",
  "url": "sec-det-properties.html#sec-det-properties-7-8-7",
  "type": "Exercise",
  "number": "4.2.5.13",
  "title": "",
...
{
  "id": "sec-det-properties-7-8-8",
  "level": "2",
  "url": "sec-det-properties.html#sec-det-properties-7-8-8",
  "type": "Exercise",
  "number": "4.2.5.14",
  "title": "",
...
},
{
  "id": "el-mat-equals-tr",
  "level": "2",
  "url": "sec-det-properties.html#el-mat-equals-tr",
  "type": "Exercise",
  "number": "4.2.5.15",
  "title": "",
  "body": "  Suppose that is a field and that is an elementary matrix which performs the scale or switch row operation. Prove that is a symmetric matrix.   "
},
{
  "id": "sec-lin-ind",
  "level": "1",
...
},
{
  ...
  "number": "5.1.5",
  "title": "",
  "body": "  In practice, we will think about linear independence in the following way. A set is linearly independent if the vector equation has only the trivial solution.  Further, when a set is linearly dependent, then we will call a non-trivial linear combination a linear dependence relation for the vectors in .   "
},
{
...
},
{
  ...
  "number": "5.1.16",
  "title": "",
  "body": "  We consider the following vectors in : . The set is linearly dependent, because the matrix which has the vectors as its columns has the following RREF: .   "
},
{
  "id": "sec-lin-ind-29",
  "level": "2",
...
},
{
  ...
  "number": "5.1.18",
  "title": "",
  "body": "  A set of vectors in is linearly independent if and only if that set spans .    Let . By , we know that spans if and only if the RREF of has a pivot in every row. Further, says that is linearly independent if and only if the RREF of has a pivot in every column. Since is a square matrix, each of these happen exactly when the RREF of is .   "
},
{
...
},
{
  ...
  "number": "5.1.20",
  "title": "",
  "body": " It is important to record that doesn't say that in linearly dependent sets every vector is a combination of the vectors with smaller subscripts. We merely have the existence of a vector with that property.  "
},
{
  "id": "sec-lin-ind-35-1",
  "level": "2",
...
},
{
  ...
  "number": "5.1.3",
  "title": "",
  "body": "  Determine the value(s) of , if any, that will make the set linearly dependent in , if .    The set is linearly dependent if and only if .   "
},
{
  "id": "sec-lin-ind-36-4",
  "level": "2",
...
},
{
  ...
  "number": "5.1.5",
  "title": "",
  "body": "  Determine whether the following statements are true or false. Justify your answer either way.   If is a subset of a vector space and is not a scalar multiple of , then is linearly independent.    If is a subset of a vector space and is not a linear combination of , , and , then is linearly independent.    If is a subset of a vector space and both and are linearly independent, then is linearly independent.    If is a subset of a vector space and is linearly independent, then is also linearly independent.          False.    False.    False.    True.      "
},
{
  "id": "sec-lin-ind-36-6",
  "level": "2",
...
},
{
  ...
  "number": "5.1.9",
  "title": "",
  "body": "  Suppose that and are subsets of a vector space . Prove that if and is linearly dependent, then is linearly dependent.    Suppose that and . Since is linearly dependent, we know that there exist scalars , not all of which are zero, such that . Then it is easy to produce a linear dependence relation for the set : . This proves that is linearly dependent.   "
},
{
  ...
  "number": "5.1.10",
  "title": "",
  "body": "  Let and let be a linear transformation.   Prove that if and only if .    Prove that is injective if and only if it is surjective.          If , then if and only if the RREF of is . It is always true that the null space is if and only if and we have a pivot in each column of the RREF; when is square, this means that the RREF must be .  We also know that if and only if the RREF of is . It is always true that the column space is if and only if and we have a pivot in each row of the RREF; when is square, this means that the RREF must be .  Putting these two paragraphs together, we conclude that if and only if .    If is a linear transformation , then by there is a matrix such that is multiplication by . Then the kernel of is the null space of and the range of is the column space of . So, is injective if and only if , and is surjective if and only if . Then the result from part a completes the argument.      "
},
{
...
},
{
  ...
  "number": "5.2.1",
  "title": "",
  "body": "  A vector space is finite-dimensional if there is a finite set of vectors that spans . A vector space is infinite-dimensional if it is not finite-dimensional.   "
},
{
  "id": "def-basis",
  "level": "2",
...
{
  "id": "subsec-intro-basis-12",
  "level": "2",
  "url": "sec-basis.html#subsec-intro-basis-12",
  "type": "Example",
  "number": "5.2.8",
  "title": "",
...
{
  "id": "subsec-intro-basis-16",
  "level": "2",
  "url": "sec-basis.html#subsec-intro-basis-16",
  "type": "Example",
  "number": "5.2.10",
  "title": "",
  "body": "  Let be the following matrix: . We will label column in as the vector .  Since the RREF of is , the set is not a basis for .  On the other hand, if is the matrix , then the columns of form a basis for since the RREF of is .   "
},
{
  "id": "thm-unique-rep",
  "level": "2",
...
},
{
  ...
  "number": "5.2.4.2",
  "title": "",
  "body": "  For each of the following, determine whether the given set of vectors forms a basis for the indicated vector space.    in if      in if      in if           No.    No.    No.      "
},
{
  "id": "sec-basis-6-3",
  "level": "2",
...
},
{
  ...
  "number": "5.2.4.4",
  "title": "",
  "body": "  Produce a matrix which has two vectors in a basis for and two vectors in a basis for .    Any matrix that has two pivot columns and two non-pivot columns will work as an answer here. This is one of many such matrices: . Of course, any matrix row equivalent to a matrix like this one would also work as an answer.   "
},
{
  "id": "sec-basis-6-5",
  "level": "2",
...
},
{
  ...
  "number": "5.2.4.6",
  "title": "",
  "body": "  Find a basis for , if .    A basis is .   "
},
{
  "id": "sec-basis-6-7",
  "level": "2",
  ...
  "number": "5.2.4.7",
  "title": "",
  "body": "  Let be the vector space of all functions . Find a basis for the subspace , if .    We will argue using the (contrapositive of the) Linear Dependence Lemma. Since is not the zero function, then is a linearly independent set. Also, since is not a scalar multiple of (this can be verified by comparing graphs of the two functions), the set is also linearly independent. However, there is a trig identity which says that , meaning that the set is linearly dependent (as one vector is a multiple of another within this set). Therefore, one basis for is .   "
},
{
  "id": "sec-basis-6-8",
  "level": "2",
...
},
{
  ...
  "number": "5.2.4.11",
  "title": "",
  "body": "  Let be a linear transformation between vector spaces, and let be a basis for .   Produce an example to show that does not need to be a basis of .    Suppose that is injective. Must be a basis for ? If so, prove it. If not, produce a counter-example.    Suppose that is surjective. Must be a basis for ? If so, prove it. If not, produce a counter-example.      "
},
{
  "id": "sec-basis-6-11-3",
  "level": "2",
  ...
  "number": "5.2.4.12",
  "title": "",
  "body": "  Suppose that is a basis for a vector space . Prove that is also a basis for .    We let . We will first show that is linearly independent.  We first suppose that are scalars such that . Rearranging this equation, we see that it is equivalent to . However, since we were given that is a basis for , this means that the coefficients in this last equation must all be zero, since is linearly independent. This means that , and then since we must also have , we have , and so on. The result is that for all , . This proves that is linearly independent.  We will now show that spans . Let . We want to argue that can be written as a linear combination of the vectors in . Since is a basis for , there exist scalars such that . We want to argue that we can always find scalars such that . This equation can be rewritten as , and by The Unique Representation Theorem ( ), we know that the coefficients on the right sides of and must be equal. Immediately we see that and then since we must have , we conclude . We can continue on in this way, eventually producing an expression for each in terms of the coefficients.  This proves that spans which concludes the proof that is a basis of .   "
},
{
  "id": "sec-basis-6-11-4",
  "level": "2",
  "url": "sec-basis.html#sec-basis-6-11-4",
  "type": "Exercise",
  "number": "5.2.4.13",
  "title": "",
  "body": "  Prove or disprove: Every basis of must contain a polynomial of degree 2, a polynomial of degree 1, and a constant polynomial.   "
},
{
  "id": "sec-basis-6-11-5",
  "level": "2",
  "url": "sec-basis.html#sec-basis-6-11-5",
  "type": "Exercise",
  "number": "5.2.4.14",
  "title": "",
  "body": "  Write down a basis for . Prove that your set is a basis. (There is no need to prove that is a vector space as this was covered in .)   "
},
{
  "id": "sec-dimension",
  "level": "1",
...
},
{
  ...
  "number": "5.3.1",
  "title": "",
  "body": "  Suppose that is a vector space and that . If is a linearly independent subset of , then .    Since for each , there exist scalars such that . We now consider the homogeneous linear system represented by the equation , where is the matrix . If is a solution to this system, then we have for all . This means that we have . Since is a linearly independent set, we must have for all , meaning that is the only solution to the equation . By (or, rather, the corresponding corollary to ), this means that , as desired.   "
},
{
  ...
  "number": "5.3.2",
  "title": "",
  "body": "  Suppose that, for every , a vector space contains a linearly independent subset of size . Then is infinite-dimensional.    We will prove the contrapositive. If is finite-dimensional, then there exists a set such that . By , for any , cannot contain a linearly independent subset of vectors. Thus, there exists a natural number such that does not contain a linearly independent subset of that size. This completes the proof of the contrapositive.   "
},
{
  "id": "infinite-dim-polys",
  "level": "2",
  "url": "sec-dimension.html#infinite-dim-polys",
  "type": "Example",
  "number": "5.3.3",
  "title": "",
...
},
{
  ...
  "number": "5.3.4.1",
  "title": "",
  "body": "  Find the dimension of the subspace of consisting of all vectors whose first and third coordinates are equal.    The dimension is 2.   "
},
{
...
},
{
  ...
  "number": "5.3.4.6",
  "title": "",
  "body": "  The first four Hermite polynomials are , , , and . Show that the set of these polynomials is a basis for .    Since and this is a set of four polynomials, we only need to argue that this set is linearly independent (see ). If we label the polynomials as , , , and , then we can argue that the set is linearly independent by the contrapositive of the Linear Dependence Lemma. The set containing only is linearly independent since . Then the set is linearly independent since neither polynomial is a scalar multiple of the other. Then since cannot be a linear combination of and for degree reasons, is linearly independent. Similarly, since cannot be a linear combination of , , and for degree reasons, is linearly independent.   "
},
{
...
},
{
  ...
  "number": "5.3.4.8",
  "title": "",
  "body": "  Let be a matrix.   Prove that is the number of non-pivot columns in .    Prove that is the number of pivot columns of .          The null space of a matrix has a basis vector for each column in the RREF which does not contain a pivot. Therefore, the dimension of the null space is the number of non-pivot columns.    By , we see that there is a vector in the basis for for each pivot in the RREF of . Therefore, is the number of pivot columns of .      "
},
{
  ...
  "number": "5.3.4.9",
  "title": "",
  "body": "  Let be the set of all functions . Prove that is infinite-dimensional.    Since contains the vector space of all polynomials, and since the vector space of all polynomials is infinite-dimensional (see ), then must be infinite-dimensional.   "
},
{
...
},
{
  ...
  "body": " Rank and Nullity   In this section we will connect dimension with the subspaces associated with linear transformations (see ).    Defining Rank and Nullity  We begin by defining the dimension of the image of a linear transformation. (Readers should recall that image and range have the same meaning, but we prefer to use image .)    Let be a linear transformation. Then the rank of , denoted , is the dimension of the image of : . The rank of a matrix is the dimension of the column space of : .    It may seem strange to define the same word in two ways. However, since the image of is exactly the column space of when is multiplication by , these two definitions coincide.  When is an matrix over , its rows are vectors in and its columns are vectors in . This is why the column space of is a subspace of . We can also examine the analogous space for the rows.    The set of all linear combinations of the rows of a matrix is called the row space of . We denote this by .     Since the rows of are the columns of , it is immediate that .   With the definition of the row space it is natural to wonder how the sizes of the row and column spaces compare to each other. The following results will help us settle this matter.    If and are row equivalent matrices, then we have . Further, if is in REF, then the non-zero rows of form a basis for (and ).    We will first show that as sets. If is reduced to , then the rows of are linear combinations of the rows of . (The elementary row operations produce linear combinations of the original rows.) Therefore, any linear combination of the rows of can be written as a linear combination of the rows of . This proves that . Since all row operations are reversible, we can use row operations to produce from , and this same argument shows that . This proves that .  If the matrix is in REF, the nonzero rows are linearly independent because no nonzero row is a linear combination of the rows below it. Here we are applying the Linear Dependence Lemma ( ) to the nonzero rows from bottom to top. Since the rows of span by definition, the fact that they are linearly independent means that they form a basis for .      Let . Then .    If we put into REF, then tells us that the number of pivots is since that is the number of vectors in a basis of . However, the number of pivots in a REF (or the RREF) of is also . (See .) This proves that .     This theorem says that . This theorem also answers the question about the relative sizes of and they are the same!     Consider the following matrix : . We will find a basis for . Here is the RREF of : . We have used the RREF of a matrix in the past to find bases for the null space and column space of a matrix. Now, we will use it to find a basis for the row space. tells us that the nonzero rows of this RREF are the basis we seek, therefore a basis for is , where .    We have defined the dimension of the image of a linear transformation , so we now turn to the kernel.    If is a linear transformation, then the nullity of is the dimension of the kernel of . If is a matrix, then the nullity of is the dimension of the null space of .     As we saw with rank, these two uses of nullity coincide for the situation when is multiplication by .  Some other texts use the notation to indicate nullity instead of null space, as we have. We will not introduce any additional notation for the nullity, but we will use or as appropriate.     The Rank-Nullity Theorem  The following theorem brings together the rank and nullity of a matrix\/linear transformation.   The Rank-Nullity Theorem   If , then . If , then .    We will prove the result for matrices. The proof for linear transformations is a bit more technical. (The reader should note that the result for linear transformations implies the result for matrices!)  If , let be the RREF of . Then is the number of pivot columns in . Further, is the number of non-pivot columns in . (See .) Since each of the columns of must be either a pivot or a non-pivot column, and since and have the same number of columns, this proves the theorem.      If is a matrix with a three-dimensional null space, this theorem tells us that the rank of is .  Let us consider an additional scenario: Could a matrix have a one-dimensional null space? If such a matrix existed, it would have a rank of , according to . But the largest rank that a matrix can have is 6, since there cannot be more pivots than there are rows. So the answer is no, a matrix cannot have a one-dimensional null space.    When the dimensions of the domain and codomain of a linear transformation are equal, some properties of such a transformation coincide.    If and , then the following are equivalent.   The transformation is injective.    The transformation is surjective.    The transformation is an isomorphism.       By , is injective if and only if . In other words, is injective if and only if . By , this happens if and only if , and if , if and only if is injective. This proves that is injective if and only if is surjective. Since a bijective linear transformation is an isomorphism, our proof is complete.    To close out this section, we present a long theorem with many equivalent statements. We will omit a proof, because the equivalence of most of these statements has been already established at various places in this text. (In other books, this theorem forms the central focus of the text. It is certainly important, but we have chosen a different emphasis.)   The Invertible Matrix Theorem   Let . Then the following statements are equivalent.   The matrix is invertible.    The matrix is row equivalent to .    The matrix has pivots.    The equation has only the trivial solution.    The columns of form a linearly independent set.    If is multiplication by , then is injective.    The equation has a solution for all .    The columns of span .    The linear transformation is surjective.    There is an matrix such that .    The matrix is invertible.    We have .    The columns of form a basis for .    We have .    We have .    We have .    We have .       This theorem ties together threads from almost every section we've covered, which is quite an achievement! The reader should note that this result only applies to square matrices.      Consider the following matrix : . Find a basis for . Explain your answer.      Suppose that is a linear transformation and that and . What are the possible values for ? Explain.        Find the rank and nullity of each of the following matrices.    ,      ,          Let be the differentiation linear transformation. Calculate and .         If a matrix has rank 3, find , , and .    If the null space of a matrix is 5-dimensional, what is the dimension of the column space of ?    If is a matrix, what is the smallest possible dimension of ?         Suppose a nonhomogeneous linear system of nine equations and ten variables has a solution for all possible constants on the right side of the equations. Is it possible to find two nonzero solutions of the associated homogeneous system that are not multiples of each other? Explain.    This is not possible. A linear system has a coefficient matrix which we will call . This situation can be reinterpreted as a linear transformation from which is multiplication by . The information given in the problem tells us that this transformation is surjective, meaning that the rank of is as large as possible, which is 9 in this case. Therefore, the Rank-Nullity Theorem says that . This means that every nonzero vector in the null space of is a scalar multiple of a single basis vector. Therefore, the associated homogeneous linear system only has nonzero solutions which are multiples of each other.      Suppose and . What has to be true about the two numbers and in order for the equation to be consistent? Explain.      Writing Exercises    If and are matrices, prove that .      Suppose that and that and are both finite-dimensional.   Prove that is surjective if and only if .    Prove that is injective if and only if .          If is surjective, then , which means that . This implies that . On the other hand, if , then . Since is a subspace of , implies that , which means that is surjective.    We will be able to argue both implications at once. A linear transformation is injective if and only if . This occurs if and only if . The Rank-Nullity Theorem then says that if and only if . This proves the result.         Suppose that is a linear system which is consistent but which does not have a unique solution. Prove that there must be a vector such that the system is inconsistent.    The given information means there is no pivot in the last column of the augmented matrix but that there is at least one non-pivot column among the first six columns. The coefficient matrix then has at least one non-pivot column, meaning that . But this means that the linear transformation which is multiplication by cannot be surjective, since the rank of is less than six. Therefore, we can find a vector such that is inconsistent.      Prove that if , then .      Prove that if , then .    We know that is the number of pivots in the RREF of , but the number of pivots must be less than both the number of columns ( ) and the number of rows ( ) of . Then if and , we have .      Let .   Prove that if , then cannot be injective.    Prove that if , then cannot be surjective.         "
},
{
  "id": "def-rank",
  "level": "2",
  ...
  "number": "5.4.1",
  "title": "",
  "body": "  Let be a linear transformation. Then the rank of , denoted , is the dimension of the image of : . The rank of a matrix is the dimension of the column space of : .   "
},
{
  "id": "def-row-space",
  "level": "2",
...
},
{
  ...
  "number": "5.4.3",
  "title": "",
  "body": " Since the rows of are the columns of , it is immediate that .  "
},
{
...
},
{
  ...
  "number": "5.4.6",
  "title": "",
  "body": " This theorem says that . This theorem also answers the question about the relative sizes of and they are the same!  "
},
{
...
},
{
  ...
  "number": "5.4.9",
  "title": "",
  "body": " As we saw with rank, these two uses of nullity coincide for the situation when is multiplication by .  Some other texts use the notation to indicate nullity instead of null space, as we have. We will not introduce any additional notation for the nullity, but we will use or as appropriate.  "
},
{
  "id": "thm-rank-nullity",
  "level": "2",
...
},
{
  ...
  "number": "5.4.3.1",
  "title": "",
  "body": "  Consider the following matrix : . Find a basis for . Explain your answer.   "
},
{
  "id": "sec-rank-5-2",
  "level": "2",
...
},
{
  ...
  "number": "5.4.4.4",
  "title": "",
  "body": "  Suppose a nonhomogeneous linear system of nine equations and ten variables has a solution for all possible constants on the right side of the equations. Is it possible to find two nonzero solutions of the associated homogeneous system that are not multiples of each other? Explain.    This is not possible. A linear system has a coefficient matrix which we will call . This situation can be reinterpreted as a linear transformation from which is multiplication by . The information given in the problem tells us that this transformation is surjective, meaning that the rank of is as large as possible, which is 9 in this case. Therefore, the Rank-Nullity Theorem says that . This means that every nonzero vector in the null space of is a scalar multiple of a single basis vector. Therefore, the associated homogeneous linear system only has nonzero solutions which are multiples of each other.   "
},
{
  "id": "sec-rank-6-5",
  "level": "2",
...
},
{
  "id": "sec-rank-6-6-2",
  "level": "2",
  "url": "sec-rank.html#sec-rank-6-6-2",
  "type": "Exercise",
  "number": "5.4.4.6",
  "title": "",
...
},
{
  "id": "sec-rank-6-6-3",
  "level": "2",
  "url": "sec-rank.html#sec-rank-6-6-3",
  "type": "Exercise",
  "number": "5.4.4.7",
  "title": "",
  "body": "  Suppose that and that and are both finite-dimensional.   Prove that is surjective if and only if .    Prove that is injective if and only if .          If is surjective, then , which means that . This implies that . On the other hand, if , then . Since is a subspace of , implies that , which means that is surjective.    We will be able to argue both implications at once. A linear transformation is injective if and only if . This occurs if and only if . The Rank-Nullity Theorem then says that if and only if . This proves the result.      "
},
{
  "id": "sec-rank-6-6-4",
  "level": "2",
  "url": "sec-rank.html#sec-rank-6-6-4",
  "type": "Exercise",
  "number": "5.4.4.8",
  "title": "",
  "body": "  Suppose that is a linear system which is consistent but which does not have a unique solution. Prove that there must be a vector such that the system is inconsistent.    The given information means there is no pivot in the last column of the augmented matrix but that there is at least one non-pivot column among the first six columns. The coefficient matrix then has at least one non-pivot column, meaning that . But this means that the linear transformation which is multiplication by cannot be surjective, since the rank of is less than six. Therefore, we can find a vector such that is inconsistent.   "
},
{
  "id": "sec-rank-6-6-5",
  "level": "2",
  "url": "sec-rank.html#sec-rank-6-6-5",
  "type": "Exercise",
  "number": "5.4.4.9",
  "title": "",
...
},
{
  "id": "sec-rank-6-6-6",
  "level": "2",
  "url": "sec-rank.html#sec-rank-6-6-6",
  "type": "Exercise",
  "number": "5.4.4.10",
  "title": "",
  "body": "  Prove that if , then .    We know that is the number of pivots in the RREF of , but the number of pivots must be less than both the number of columns ( ) and the number of rows ( ) of . Then if and , we have .   "
},
{
  "id": "sec-rank-6-6-7",
  "level": "2",
  "url": "sec-rank.html#sec-rank-6-6-7",
  "type": "Exercise",
  "number": "5.4.4.11",
  "title": "",
...
},
{
  ...
  "number": "5.5.11",
  "title": "",
  "body": " There are several linear transformations involved in the transformation , multiplication by the matrix , and the two coordinate mappings. Because it is easy to confuse the roles of these transformations, mathematicians frequently employ a tool called a commutative diagram to keep their ideas and symbols organized.  We will produce here the diagram that summarizes the conclusion of .   A commutative diagram illustrating the relationship between a linear transformation and multiplication by a matrix.   A commutative diagram illustrating the previous proposition. See long description.   A rectangular commutative diagram. The upper left corner contains with a horizontal arrow to the upper right corner containing ; this arrow is labeled with . An arrow labeled is drawn from the in the upper left to in the lower left corner. An arrow labeled is drawn from the in the upper right to in the lower right. An arrow labeled is drawn from the lower left to the lower right.      We have labeled the coordinate mappings from to and from to by and , respectively. Further, we have labeled the linear transformation which is multiplication by , from to , with .  The proposition claims that this diagram commutes . In other words, for any vector , going around the rectangle in either of the possible directions (right then down or down then right) will produce the same result. This would mean that . With different notation, this is exactly the conclusion of .  "
},
{
...
{
  "id": "subsec-coordinates-matrices-9",
  "level": "2",
  "url": "sec-coordinates.html#subsec-coordinates-matrices-9",
  "type": "Example",
  "number": "5.5.14",
  "title": "",
  "body": "  We consider a linear transformation defined by . We let be the standard basis for and be the standard basis for . We now write for each : . These coordinate vectors make up the columns of the matrix . If we wanted to calculate , where , we could do so using coordinate vectors and the matrix . Since the coordinate vector of with respect to is fairly obvious it is itself we can proceed with this calculation: . This tells us that .   "
},
{
  ...
  "number": "5.5.15",
  "title": "",
  "body": "  Let , , and be finite-dimensional vector spaces with bases , , and , respectively. Suppose that and that . Then .    We assume that , , and . Further, we let , , and . We want to show that .  The definition of tells us that, for each , the th column of is the coordinate vector of with respect to . So . Also, for each , . Using the linearity of these transformations, we have . This means that for all and . Since this is how the -entry of the matrix product is formed, this proves that , as desired. This proof was taken from Linear Algebra (2018), Meckes and Meckes, pages 193-194.    "
},
{
...
},
{
  ...
  "number": "5.5.4.6",
  "title": "",
  "body": "  Use coordinate vectors to test whether the following sets of vectors span .    if      if           This set of vectors spans .    This set of vectors does not span .      "
},
{
...
},
{
  ...
  "number": "5.5.4.11",
  "title": "",
  "body": "  Consider the plane in defined by .   Find a basis for .    Determine whether each of the following vectors is in , and for each one that is, find its coordinate vector in terms of the basis you gave in part a.                       "
},
{
  "id": "sec-coordinates-6-12-2",
  "level": "2",
  "url": "sec-coordinates.html#sec-coordinates-6-12-2",
  "type": "Exercise",
  "number": "5.5.4.12",
  "title": "",
  "body": "  Prove that the coordinate mapping in is a linear transformation.   "
},
{
  "id": "sec-coordinates-6-12-3",
  "level": "2",
  "url": "sec-coordinates.html#sec-coordinates-6-12-3",
  "type": "Exercise",
  "number": "5.5.4.13",
  "title": "",
  "body": "  Without using , prove that the coordinate mapping in is injective.    Let be an -dimensional vector space over , and let be a basis for . Let be the coordinate mapping. We will prove that is injective by showing that it has a trivial kernel.  Suppose that . This means that , so . Since this is the coordinate vector of , this tells us that . This proves that , and therefore . This proves that the coordinate mapping is injective.   "
...
{
  "id": "sec-coordinates-6-12-4",
  "level": "2",
  "url": "sec-coordinates.html#sec-coordinates-6-12-4",
  "type": "Exercise",
  "number": "5.5.4.14",
  "title": "",
  "body": "  Without using , prove that the coordinate mapping in is surjective.   "
},
{
  "id": "sec-coordinates-6-12-5",
  "level": "2",
  "url": "sec-coordinates.html#sec-coordinates-6-12-5",
  "type": "Exercise",
  "number": "5.5.4.15",
  "title": "",
  "body": "  Let be a linear transformation between finite-dimensional vector spaces, and let and be bases for and , respectively. Prove that (the rank of as a linear transformation) is the same as (the rank of the coordinate matrix of ).   "
},
{
  "id": "sec-coordinates-6-12-6",
  "level": "2",
  "url": "sec-coordinates.html#sec-coordinates-6-12-6",
  "type": "Exercise",
  "number": "5.5.4.16",
  "title": "",
  "body": "  Prove .   "
...
},
{
  ...
  "number": "5.6.3.1",
  "title": "",
  "body": "  Let and be bases of , where . Find the change-of-basis matrices and .    Here are the change-of-basis matrices: .   "
},
{
...
},
{
  ...
  "number": "5.6.3.3",
  "title": "",
  "body": "  Consider the basis of where .   If is the standard basis for , find and .    Use your work in part (a) to find if .          Here are the change-of-basis matrices: . (Yes, they are the same!)    We find that .      "
},
{
  ...
  "number": "5.6.3.4",
  "title": "",
  "body": "  Let be a basis for , where .   If is the standard basis for , find and .    Use your work in part (a) to find if .    If is the linear transformation that takes the derivative, find .      "
},
{
  ...
  "number": "5.6.3.5",
  "title": "",
  "body": "  Let be a basis for , where .    If is the standard basis for , find and .    If is the linear transformation , find .     "
},
{
...
{
  "id": "sec-change-of-basis-5-7",
  "level": "2",
  "url": "sec-change-of-basis.html#sec-change-of-basis-5-7",
  "type": "Exercise",
  "number": "5.6.3.7",
  "title": "",
...
{
  "id": "sec-change-of-basis-5-8",
  "level": "2",
  "url": "sec-change-of-basis.html#sec-change-of-basis-5-8",
  "type": "Exercise",
  "number": "5.6.3.8",
  "title": "",
  "body": "  Consider the linear transformation which is reflection across the line .   Propose a basis for where will be easy to determine.    Find .    If is the standard basis for , find and .    Using your work in previous parts of this problem, find .          We will use the basis , where .         We find that .           "
},
{
  "id": "sec-change-of-basis-5-9-2",
  "level": "2",
  "url": "sec-change-of-basis.html#sec-change-of-basis-5-9-2",
  "type": "Exercise",
  "number": "5.6.3.9",
  "title": "",
  "body": "  Let be an -dimensional vector space over , and let and be two bases for . Prove that the columns of the matrix are linearly independent.   "
},
{
  "id": "sec-change-of-basis-5-9-3",
  "level": "2",
  "url": "sec-change-of-basis.html#sec-change-of-basis-5-9-3",
  "type": "Exercise",
  "number": "5.6.3.10",
  "title": "",
  "body": "  Let be an -dimensional vector space over , and let and be two bases for . Prove that the columns of the matrix span .    The th column of is . Or, stated differently, if is the coordinate mapping, then the th column of is .  We know (by ) that the coordinate mapping is an isomorphism. Since is a basis for , is a spanning set for . But then the set containing the columns of is , and since spans we know that will span .   "
},
{
  "id": "sec-change-of-basis-5-9-4",
  "level": "2",
  "url": "sec-change-of-basis.html#sec-change-of-basis-5-9-4",
  "type": "Exercise",
  "number": "5.6.3.11",
  "title": "",
  "body": "  Let be invertible. Prove that there exist bases and for such that .   "
},
{
  "id": "sec-eigenvalues",
  "level": "1",
...
},
{
  ...
  "number": "6.1.8",
  "title": "",
  "body": "  Let be a vector space and let . If is an eigenvalue of , then the eigenspace of corresponding to is the subspace of defined by . We may refer to the eigenspace corresponding to as the -eigenspace .   "
},
{
  "id": "examp-evalues1",
  "level": "2",
...
},
{
  ...
  "number": "6.1.4.3",
  "title": "",
  "body": "  Let be the following matrix .   Show that is an eigenvalue for and find a basis for .    Show that is an eigenvalue for and find a basis for .          The RREF of has one non-pivot column, so is an eigenvalue for . A basis for is .    The RREF of has two non-pivot columns, so is an eigenvalue for . A basis for is .      "
},
{
...
},
{
  ...
  "number": "6.1.4.5",
  "title": "",
  "body": "  Let be the following matrix .   Show that is an eigenvalue for and find a basis for .    Show that is an eigenvalue for and find a basis for .          The RREF of has two non-pivot columns, so is an eigenvalue for . A basis for is .    The RREF of has one non-pivot column, so is an eigenvalue for . A basis for is .      "
},
{
...
},
{
  ...
  "number": "6.1.4.10",
  "title": "",
  "body": "  Prove that if is the zero matrix, then the only eigenvalue of is 0.    Suppose that is an eigenvalue of . Then there exists a nonzero vector such that . But then . If , then , so we have . But since is nonzero, and since is a field, we must have . Therefore, the only eigenvalue of is 0.   "
},
{
  ...
  "number": "6.1.4.11",
  "title": "",
  "body": "  Prove that an matrix can have at most distinct eigenvalues.    We will argue by contradiction. Suppose that an matrix has distinct eigenvalues. This means that there are corresponding eigenvectors . By , we know that the set is linearly independent. However, this gives us a linearly independent set of vectors in , which is a contradiction by . This proves that can have at most distinct eigenvalues.   "
},
{
...
},
{
  ...
  "number": "6.2.6",
  "title": "",
  "body": "  The algebraic multiplicity of an eigenvalue is the number of times it appears as a root of the characteristic polynomial for a matrix. We will often refer to this number as an eigenvalue's multiplicity .   "
},
{
...
},
{
  ...
  "number": "6.2.13",
  "title": "",
  "body": "  Let be the following matrices: . Since both and are triangular matrices, we can see that they have the same eigenvalue (with the same multiplicity). However, these matrices are not similar. In fact, we will show in the exercises that a matrix , for , is only similar to itself. In this example, .   "
},
{
  "id": "sec-char-eqn-5-1",
  "level": "2",
...
},
{
  ...
  "number": "6.2.3.2",
  "title": "",
  "body": "  Consider the matrix from the previous reading question. Write down two separate matrices which are similar to . Explain how you obtained these matrices. (There are many, many correct answers!)    "
},
{
  "id": "sec-char-eqn-6-1",
  "level": "2",
...
},
{
  ...
  "number": "6.2.4.2",
  "title": "",
  "body": "  Let be the following matrix: . Find the characteristic polynomial and the eigenvalues of .    The characteristic polynomial is . The eigenvalues are and .   "
},
{
  "id": "sec-char-eqn-6-3",
  "level": "2",
...
},
{
  ...
  "number": "6.2.4.4",
  "title": "",
  "body": "  Let be the following matrix: . Find the characteristic polynomial and the eigenvalues of .    The characteristic polynomial is , and the eigenvalues are and .   "
},
{
  "id": "sec-char-eqn-6-5",
  "level": "2",
  ...
  "number": "6.2.4.5",
  "title": "",
  "body": "  Let be the following matrix: . Find the characteristic polynomial and the eigenvalues of by hand.   "
},
{
  "id": "sec-char-eqn-6-6",
  "level": "2",
  ...
  "number": "6.2.4.6",
  "title": "",
  "body": "  Let be the following matrix: . Find the characteristic polynomial and the eigenvalues of by hand.    The characteristic polynomial is , but it is easier in this problem NOT to expand the calculation to this form. The characteristic polynomial as calculated is . The eigenvalues are and .   "
},
{
  "id": "sec-char-eqn-6-7",
  "level": "2",
  ...
  "number": "6.2.4.7",
  "title": "",
  "body": "  Let be the following matrix: . Find the characteristic polynomial of by hand. If is an eigenvalue of , find the other eigenvalues of by hand.   "
},
{
  "id": "sec-char-eqn-6-8",
  "level": "2",
  "url": "sec-char-eqn.html#sec-char-eqn-6-8",
  "type": "Exercise",
  "number": "6.2.4.8",
  "title": "",
  "body": "  Let be the following matrix: . Find the characteristic polynomial of by hand. If is an eigenvalue of , find the other eigenvalues of by hand.    The characteristic polynomial is . The eigenvalues are , , and .   "
},
{
  "id": "sec-char-eqn-6-9-2",
  "level": "2",
  "url": "sec-char-eqn.html#sec-char-eqn-6-9-2",
  "type": "Exercise",
  "number": "6.2.4.9",
  "title": "",
...
},
{
  ...
  "number": "6.2.4.10",
  "title": "",
  "body": "  Prove that similarity is an equivalence relation on the set .    For the purposes of notation, we will say that if is similar to . We first note that a matrix is similar to itself since , where is the identity matrix. This proves that the relation is reflexive.  We now suppose that with . This means that there is an invertible matrix such that . We can manipulate this equation to give , and since is invertible (and since ), this proves that . This proves that the relation is symmetric.  We now suppose that with and . This means that we have invertible matrices and such that and . Putting these equations together, we have . In this derivation we have used . Since is invertible (again by ), this proves that , which shows that the relation is transitive.  We have shown that the relation of similarity on the set is reflexive, symmetric, and transitive, so it is an equivalence relation.   "
},
{
  "id": "sec-char-eqn-6-9-4",
  "level": "2",
  "url": "sec-char-eqn.html#sec-char-eqn-6-9-4",
  "type": "Exercise",
  "number": "6.2.4.11",
  "title": "",
...
{
  "id": "sec-char-eqn-6-9-6",
  "level": "2",
  "url": "sec-char-eqn.html#sec-char-eqn-6-9-6",
  "type": "Exercise",
  "number": "6.2.4.13",
  "title": "",
  "body": "  Let . Prove that if and only if .   "
},
{
...
},
{
  "id": "def-geo-mult",
  "level": "2",
  "url": "sec-diagonalization.html#def-geo-mult",
  "type": "Definition",
  "number": "6.3.11",
  "title": "",
  "body": "  The geometric multiplicity of an eigenvalue of a matrix is the dimension of the eigenspace .   "
},
{
...
},
{
  ...
  "number": "6.3.4.1",
  "title": "",
  "body": "  Consider the following matrix : .   Find the characteristic polynomial and the eigenvalues of . Show your work.     Using only part a (this means you should make no additional calculations), explain why is diagonalizable.       "
},
{
...
},
{
  ...
  "level": "2",
  "url": "sec-diagonalization.html#sec-diagonalization-7-8-2",
  "type": "Exercise",
  "number": "6.3.5.8",
  "title": "",
...
{
  "id": "sec-diagonalization-7-8-3",
  "level": "2",
  "url": "sec-diagonalization.html#sec-diagonalization-7-8-3",
  "type": "Exercise",
  "number": "6.3.5.9",
  "title": "",
...
},
{
  ...
  "level": "2",
  "url": "sec-diagonalization.html#sec-diagonalization-7-8-4",
  "type": "Exercise",
  "number": "6.3.5.10",
  "title": "",
...
{
  "id": "sec-diagonalization-7-8-5",
  "level": "2",
  "url": "sec-diagonalization.html#sec-diagonalization-7-8-5",
  "type": "Exercise",
  "number": "6.3.5.11",
  "title": "",
...
{
  "id": "sec-diagonalization-7-8-6",
  "level": "2",
  "url": "sec-diagonalization.html#sec-diagonalization-7-8-6",
  "type": "Exercise",
  "number": "6.3.5.12",
  "title": "",
...
{
  "id": "sec-diagonalization-7-8-7",
  "level": "2",
  "url": "sec-diagonalization.html#sec-diagonalization-7-8-7",
  "type": "Exercise",
  "number": "6.3.5.13",
  "title": "",
...
},
{
  ...
  "number": "6.4.4",
  "title": "",
  "body": "  Let be a positive integer and let be a field. The determinant and the characteristic polynomial are both invariants on the set with respect to the relation of similarity.   "
},
{
...
},
{
  ...
  "number": "6.4.4.2",
  "title": "",
  "body": "  Consider the following two matrices in : .   Explain why both trace and rank are not useful invariants to say that and are not similar.    Prove that and are not similar.          Since and , neither the rank nor the trace can tell us whether or not and are similar matrices.    We can calculate that while , so this means that and are not similar.      "
},
{
  ...
  "number": "6.4.4.3",
  "title": "",
  "body": "  Consider the following two matrices in : .   Explain why the trace, rank, and determinant are not useful invariants to say that and are not similar.    Prove that and are not similar.           We find that , , and , so the trace, rank, and determinant do not give us enough information to determine whether or not and are similar.    Some calculation shows that the eigenvalues for are , while the eigenvalues for are . Since these are different (and therefore the characteristic polynomials are different), we see that and are not similar.      "
},
{
  ...
  "number": "6.4.4.4",
  "title": "",
  "body": "  Consider the following two matrices in : .   Explain why the trace and rank are not useful invariants to say that and are not similar.    Prove that and are not similar.       "
},
{
  ...
  "number": "6.4.4.5",
  "title": "",
  "body": "  Consider the following two matrices in : .   Explain why the trace, rank, and determinant are not useful invariants to say that and are not similar.    Prove that and are not similar.       "
},
{
  ...
  "number": "6.4.4.6",
  "title": "",
  "body": "  Consider the following two matrices in : .   Explain why the trace, rank, determinant, and characteristic polynomial are not useful invariants to say that and are not similar.    Prove that and are not similar.           We find that , , and . Further, the eigenvalues for both and are . These four invariants do not provide enough information to determine whether or not and are similar.    We can show that is not diagonalizable but is. Since diagonalizability is an invariant for similarity, this proves that and are not similar.      "
},
{
...
{
  "id": "sec-invariants-6-8-2",
  "level": "2",
  "url": "sec-invariants.html#sec-invariants-6-8-2",
  "type": "Exercise",
  "number": "6.4.4.8",
  "title": "",
...
{
  "id": "sec-invariants-6-8-3",
  "level": "2",
  "url": "sec-invariants.html#sec-invariants-6-8-3",
  "type": "Exercise",
  "number": "6.4.4.9",
  "title": "",
  "body": "  Fix a positive integer . Is diagonalizability an invariant (for similarity) on the set ? In other words, for , if and are similar, must it be true that either (a) they are both diagonalizable, or (b) they are both not diagonalizable?   "
},
{
  "id": "sec-invariants-6-8-4",
  "level": "2",
  "url": "sec-invariants.html#sec-invariants-6-8-4",
  "type": "Exercise",
  "number": "6.4.4.10",
  "title": "",
...
{
  "id": "sec-invariants-6-8-5",
  "level": "2",
  "url": "sec-invariants.html#sec-invariants-6-8-5",
  "type": "Exercise",
  "number": "6.4.4.11",
  "title": "",
...
{
  "id": "sec-invariants-6-8-6",
  "level": "2",
  "url": "sec-invariants.html#sec-invariants-6-8-6",
  "type": "Exercise",
  "number": "6.4.4.12",
  "title": "",
...
{
  "id": "sec-invariants-6-8-7",
  "level": "2",
  "url": "sec-invariants.html#sec-invariants-6-8-7",
  "type": "Exercise",
  "number": "6.4.4.13",
  "title": "",
...
{
  "id": "sec-invariants-6-8-8",
  "level": "2",
  "url": "sec-invariants.html#sec-invariants-6-8-8",
  "type": "Exercise",
  "number": "6.4.4.14",
  "title": "",
  "body": "  Is the trace an invariant for row equivalence? Justify your answer.    No, trace is not an invariant for row equivalence. Consider the following matrix : . The matrix is invertible, so it is row equivalent to . However, and . This proves that trace is not an invariant for row equivalence.   "
},
{
  "id": "sec-invariants-6-8-9",
  "level": "2",
  "url": "sec-invariants.html#sec-invariants-6-8-9",
  "type": "Exercise",
  "number": "6.4.4.15",
  "title": "",
...
{
  "id": "sec-invariants-6-8-10",
  "level": "2",
  "url": "sec-invariants.html#sec-invariants-6-8-10",
  "type": "Exercise",
  "number": "6.4.4.16",
  "title": "",
...
{
  "id": "sec-invariants-6-8-11",
  "level": "2",
  "url": "sec-invariants.html#sec-invariants-6-8-11",
  "type": "Exercise",
  "number": "6.4.4.17",
  "title": "",
  "body": "  Is invertibility an invariant for similarity? Justify your answer.    Yes, invertibility is an invariant for similarity. This argument relies on the fact that the determinant is an invariant for similarity.  We will denote the similarity relation by . Suppose that and that is invertible. This means that , and since , this means that also. But the fact that means that is invertible.  We now suppose that and that is not invertible. This means that , and since , this means that . But means that is not invertible.  This proves that invertibility is an invariant for similarity.   "
},
{
...
},
{
  ...
  "number": "7.1.26",
  "title": "",
  "body": "  We consider two vectors in to understand the relationship in this lemma: . The lemma specifies our calculations: . We will verify the properties specified by the lemma. Since , we have . Additionally, since , we see that .   "
},
{
...
},
{
  ...
  "number": "7.1.6.2",
  "title": "",
  "body": "  Use to prove that the following set of vectors in is linearly independent: , where .    Calculation shows that the set is orthogonal, so then it is linearly independent.   "
},
{
  ...
  "number": "7.1.6.3",
  "title": "",
  "body": "  Consider the following inner product on : .   Give an example of two vectors in which are orthogonal with respect to the dot product but which are not orthogonal with respect to this inner product.    Give an example of two vectors in which are orthogonal with respect to this inner product but which are not orthogonal with respect to the dot product.           We let and be the following vectors: . Then it is easy to see that , but .    We let and be the following vectors: . Then it is easy to see that , but .      "
},
{
  "id": "ip-A-R2",
  "level": "2",
  "url": "sec-inner-products.html#ip-A-R2",
  "type": "Exercise",
  "number": "7.1.6.4",
  "title": "",
...
},
{
  "id": "trace-ip-exer",
  "level": "2",
  "url": "sec-inner-products.html#trace-ip-exer",
  "type": "Exercise",
  "number": "7.1.6.5",
  "title": "",
  "body": "  Define the following function on : . (This function defines an inner product, but you do not need to prove this right now.)   Let and be the following matrices: . Calculate using this inner product.    Calculate and for the matrices and given in part a.    Find a matrix which is orthogonal to the matrix (given in part a.) with respect to this inner product.      "
},
{
  ...
  "number": "7.1.6.6",
  "title": "",
  "body": "  Consider the following function defined on : . Show that this function is not an inner product.    Let . It is fairly easy to see that , so we have . However, since is not the zero matrix (i.e., the zero vector for the vector space ), the fourth axiom of the inner product does not hold for this function.   "
},
{
  ...
  "number": "7.1.6.7",
  "title": "",
  "body": "  Consider the following function defined on : . Show that this function is not an inner product.   "
},
{
  "id": "sec-inner-products-8-8-2",
  "level": "2",
  "url": "sec-inner-products.html#sec-inner-products-8-8-2",
  "type": "Exercise",
  "number": "7.1.6.8",
  "title": "",
  "body": "  Consider the following function defined on : . Prove or disprove: this function is an inner product.    This function is not an inner product. Consider the vector . We have . However, since is not the zero vector in , the fourth axiom of the inner product does not hold.   "
},
{
  "id": "sec-inner-products-8-8-3",
  "level": "2",
  "url": "sec-inner-products.html#sec-inner-products-8-8-3",
  "type": "Exercise",
  "number": "7.1.6.9",
  "title": "",
  "body": "  Prove that the function given in is an inner product.   "
},
{
  "id": "sec-inner-products-8-8-4",
  "level": "2",
  "url": "sec-inner-products.html#sec-inner-products-8-8-4",
  "type": "Exercise",
  "number": "7.1.6.10",
  "title": "",
  "body": "  Prove that the function given in is an inner product.   "
...
{
  "id": "sec-inner-products-8-8-5",
  "level": "2",
  "url": "sec-inner-products.html#sec-inner-products-8-8-5",
  "type": "Exercise",
  "number": "7.1.6.11",
  "title": "",
...
{
  "id": "sec-inner-products-8-8-6",
  "level": "2",
  "url": "sec-inner-products.html#sec-inner-products-8-8-6",
  "type": "Exercise",
  "number": "7.1.6.12",
  "title": "",
  "body": "  Prove that the inner product defined in is an inner product.    We will show that all four of the inner product axioms hold. For the first axiom, we note that everything here is real-valued, so we do not need to worry about any complex conjugates. Since the order of the functions within a definite integral can be switched, we have . This shows that the first axiom holds.  For the second axiom, we let . The definite integral is linear with respect to the sum of functions, so we have . This proves that the second axiom holds.  Let and let . The definite integral is linear with respect to scalar multiplication by a real number, so we have . This proves that the third axiom holds.  Let . We observe that is a function with values that are always non-negative (since each value of this function is a real number squared). Since the definite integral can be interpreted as calculating signed area between the graph of a function and the -axis, we know that . Finally, using this same signed area interpretation of the definite integral, the only way a non-negative function could produce a zero value for the definite integral is if the function was uniformly the zero function. This means that if , we must have and therefore . This proves that the fourth axiom holds.   "
},
{
  "id": "sec-inner-products-8-8-7",
  "level": "2",
  "url": "sec-inner-products.html#sec-inner-products-8-8-7",
  "type": "Exercise",
  "number": "7.1.6.13",
  "title": "",
...
{
  "id": "sec-inner-products-8-8-8",
  "level": "2",
  "url": "sec-inner-products.html#sec-inner-products-8-8-8",
  "type": "Exercise",
  "number": "7.1.6.14",
  "title": "",
...
},
{
  ...
  "body": " The Gram-Schmidt Process   This process results in an orthonormal basis for any finite-dimensional inner product space .   Start with a basis of .    Define .    For , define recursively by and .   Then is an orthonormal basis of . Additionally, for each , we have .   "
},
{
...
},
{
  ...
  "number": "7.2.5.2",
  "title": "",
  "body": "  Consider the basis for , where . Consider the inner product on defined by . (You do not need to prove that this is an inner product.)   Use the Gram-Schmidt process to create an orthonormal basis from .    Let . Use to find the coordinate vector of with respect to the orthonormal basis of you created in part (a).          The orthonormal basis we obtain is .    The coordinate vector is .      "
},
{
  ...
  "number": "7.2.5.3",
  "title": "",
  "body": "  Carry out the Gram-Schmidt process on the following set of vectors in . Use the standard dot product in : .    The orthonormal basis we obtain is .   "
},
{
...
{
  "id": "sec-orthonormal-bases-7-6-2",
  "level": "2",
  "url": "sec-orthonormal-bases.html#sec-orthonormal-bases-7-6-2",
  "type": "Exercise",
  "number": "7.2.5.6",
  "title": "",
...
},
{
  ...
  "number": "7.3.5",
  "title": "",
  "body": "  Let be a finite-dimensional subspace of an inner product space .   The function is a linear transformation.    If is an orthonormal basis of , then for each .    For each , .    For each , .    If is an orthonormal basis for , and is an orthonormal basis of (with ), then is a diagonal matrix with the first diagonal entries being 1 and the remaining diagonal entries being 0.    We have and for all .    We have .    If is finite-dimensional, then .    We have .       We will prove property 1. Let , so we can write and , for and . Then , which tells us that . We now let and . We write , with and . We note that and since and are subspaces. Then . This proves that is a linear transformation.  We will also prove property 3. Let and write , with and . Then .  We leave the proof of the other properties to the exercises.   "
},
{
  ...
  "number": "7.3.6",
  "title": "",
  "body": "  Let be a subspace of or with orthonormal basis . Then the matrix of with respect to the standard basis is , where denotes the conjugate transpose of a matrix.    This fact follows from part 2 of and the fact that the standard inner product in can be written as , where matrix multiplication is in view on the right side of the equals sign.   "
},
{
...
},
{
  ...
  "number": "7.3.11",
  "title": "",
  "body": "  Consider the following set of five points in : . Our subspace is spanned by and , where is the vector of first coordinates . We form the matrix with columns and . Then, by , we have , where is the vector of the -coordinates of our data.  We calculate the following: . Now, we don't actually want , because that is a vector in . We want to know the coefficients and in the linear combination of the column vectors of which produce . In other words, we want the vector . Since , this means that and . We can see that this is a believable solution by looking at the graph below which contains the five points as well as the line .   Graph of least-squares regression line with data   The graph of five data points and the least-squares regression line. See long description.   The graph of five data points and the least-squares regression line plotted on coordinate axes. The horizontal axis ranges from to and the vertical axis ranges from to . The five data points , , , , and are marked with solid black circles. The regression line is drawn in red from to .       "
},
{
  ...
  "number": "7.3.4.1",
  "title": "",
  "body": "  Let be the line in . (This is a subspace of .)   Calculate an orthonormal basis for . (We are considering with the usual dot product.)     Let . Using part 2 of , calculate .       "
},
{
  ...
  "number": "7.3.4.2",
  "title": "",
  "body": "  Consider the same situation as in the first reading question. Using , find the matrix of with respect to the standard basis of .   "
},
{
  ...
  "number": "7.3.5.1",
  "title": "",
  "body": "  Let be the line in . Write the vector as the sum of a vector in and a vector in . (Use the standard dot product as the inner product in .)    If we let and be the following vectors, , then where and .   "
},
{
  ...
  "number": "7.3.5.2",
  "title": "",
  "body": "  Let , where .   Find the matrix . (Use the standard dot product as the inner product in .)    Using your work from part (a), find the vector in which is closest to , if .          Our matrix is .    The vector in which is closest to is .      "
},
{
  ...
  "number": "7.3.5.3",
  "title": "",
  "body": "  Consider the following inner product on : . Let . If , write as the sum of a vector in and a vector in .    Let and be the following polynomials: . Then , where and .   "
},
{
...
{
  "id": "sec-orthogonal-projections-7-5-2",
  "level": "2",
  "url": "sec-orthogonal-projections.html#sec-orthogonal-projections-7-5-2",
  "type": "Exercise",
  "number": "7.3.5.5",
  "title": "",
  "body": "  Let be a subspace of an inner product space . Prove that is a subspace of .    First, we know that for all by . This proves that .  Next, we let and . Then, by the properties of the inner product, we have . This shows that , meaning that is closed under addition.  Finally, we let and . (Here, is either or .) We also let . Then, by the properties of the inner product, we have . This proves that so that is closed under scalar multiplication.  Since contains the zero vector and is closed under addition and scalar multiplication, is a subspace of .   "
},
{
  "id": "sec-orthogonal-projections-7-5-3",
  "level": "2",
  "url": "sec-orthogonal-projections.html#sec-orthogonal-projections-7-5-3",
  "type": "Exercise",
  "number": "7.3.5.6",
  "title": "",
  "body": "  Let . Prove that .     We will prove that each of these sets is a subset of the other. First, we let , so that . The fact that means that for all rows of . (Entry in is the dot product of the th row of with .) Since the rows of span , the fact that for each row of means that for all . This proves that , so .  We now let . We want to show that . Since , we know that for each row of . This shows that , which proves that . Therefore, .  Since we have shown that and , we can conclude that , as desired.   "
},
{
  "id": "apdx-mod-arith",
  "level": "1",
